...
function UnBotCloseAll()
	...
	NPCFrame:Hide();
	UnBotFrame:Hide();
	DisplayInfomation("Close UnBot action bar. Type /unbot in chat to reopen.");
end
...
function InitializeUnBotFrame()
	...
	InitializeStrategy();
	DisplayInfomation("Le contrôleur du robot a été initialisé avec succès");
end

local function GetCommandTypeTextByType(typeIndex)
	if (typeIndex == 4) then
		return "Affecte toute l'équipe";
	elseif (typeIndex == 3) then
		return "Nécessite une cible ennemie";
	elseif (typeIndex == 2) then
		return "Nécessite une cible alliée";
	else
		return "Ne nécessite pas de cible";
	end
end
...
function CommandButton_OnEnter(self,index,btnType)
	...
	if (self.groupIndex > 0) then
		GameTooltip:AddDoubleLine("Commande :",UnBotExecuteCommand[index],0,0.85,0.85,0,0.85,0.85);
	end
	GameTooltip:AddLine("Clic gauche : Exécuter la commande",0.65,0.55,0,1);
	if (self.groupIndex > 0) then
		if (btnType == 1) then
			GameTooltip:AddLine("Clic droit : Afficher/Masquer le menu contextuel",0.65,0.55,0,1);
		elseif (btnType == 2) then
			GameTooltip:AddLine("Clic droit : Assigner au raccourci",0.65,0.55,0,1);
		end
	else
		GameTooltip:AddLine("Clic droit : Fermer la barre d'action",0.65,0.55,0,1);
	end
	GameTooltip:AddDoubleLine("ID de commande :",tostring(index),0,0,1,1,0,1);
