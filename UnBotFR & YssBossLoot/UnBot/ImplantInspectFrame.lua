	...
	if (slot17Linnk == nil and slot16Linnk ~= nil) then
		...
		if (item ~= nil and item[11] ~= "INVTYPE_2HWEAPON") then
			...
			if (selectItem ~= nil) then
				...
				--DisplayInfomation("Need up adjoint "..tostring(selectItem[5])..", LV "..tostring(UnitLevel(targetName)));
			end
		end
	end
end
...
function UnBotInitInspectFrame()
	...
	DisplayInfomation("L'intégration du module dans la fenêtre de visualisation des personnages est terminée.");
end
...
function UnBotInspectPaperDollFrame_OnShow(...)
	...
	if (dstFrame.ubHelpText == nil) then
		...
		dstFrame.ubHelpText:SetText("Dans l'emplacement d'équipement : faites un clic gauche avec la souris pour changer d'équipement, et faites un clic droit avec la souris pour retirer l'équipement.");
