...
function UnBotGetCostEnergyText(costType, costValue)
	...
	if (costType == 0) then
		return "consommer"..tostring(costValue).."mana";
	elseif (costType == 1) then
		return "consommer"..tostring(costValue).."rage";
	elseif (costType == 3) then
		return "consommer"..tostring(costValue).."énergie";
	else
		return "consommer"..tostring(costValue).."ressource";
	end
end
	--item [2] = spellID
	--item [3] = name
			...
			if (needQuery == false) then
				if (fromParent.bagsType == 1) then
					...
					if (fromParent.dataGroup[newFrame.dataIndex][7] ~= nil and fromParent.dataGroup[newFrame.dataIndex][7] > 1) then
						...
						GameTooltip:AddDoubleLine("Quantité possédée：",tostring(fromParent.dataGroup[newFrame.dataIndex][7]),0,0.8,0.8,0.8,0.8,0);
					end
				elseif (fromParent.bagsType == 2) then
					...
					if (spellLink ~= nil) then
						...
						if (spellData[10] <= 0) then
							castDis = "auto-casting";
						else
							castDis = tostring(spellData[10]).."distance en mètres";
						end
						if (spellData[9] <= 0) then
							GameTooltip:AddDoubleLine("lancer immédiatement",castDis,0.65,0.55,0,0,0.8,0.8);
						else
							GameTooltip:AddDoubleLine(tostring(spellData[9]/1000).."Coulé en quelques secondes",castDis,0.65,0.55,0,0,0.8,0.8);
						end
					end
				end
			else
				GameTooltip:AddLine(fromParent.dataGroup[newFrame.dataIndex][3],1,0,0,1);
				GameTooltip:AddLine("Cet article n'apparaît pas dans votre sac à dos，Besoin d'interroger le serveur，Veuillez cliquer sur le bouton Actualiser en bas de l'interface pour l'afficher à nouveau.",1,0,0,1);
			end
			GameTooltip:AddLine(" ",1,1,1,1);
			if (fromParent.command ~= nil and fromParent.command ~= "") then
				GameTooltip:AddLine("clic gauche de la souris：laisser"..fromParent.target.." "..fromParent.activeText,0.65,0.55,0,1);
			end
			if (fromParent.bagsType == 1) then
				GameTooltip:AddLine("clic droit de la souris：cacher cet accessoire",0.65,0.55,0,1);
			elseif (fromParent.bagsType == 2) then
				GameTooltip:AddLine("clic droit de la souris：Cacher cette compétence",0.65,0.55,0,1);
			else
				GameTooltip:AddLine("clic droit de la souris：Masquer cette icône",0.65,0.55,0,1);
			end
			if (fromParent.bagsType == 2) then
				GameTooltip:AddDoubleLine("ID de sort：",tostring(itemID),0,0.8,0.8,0.8,0,0);
			end
		else
			GameTooltip:AddLine(newFrame.bagsIcon);
		end
		GameTooltip:AddDoubleLine("Ordre d'indexation：",tostring(newFrame.iconIndex),0,0,1,1,0,1);
...
local function CreateBagsTypeOptions(fromParent, checkedIndex)
	...
	newFrame.title:SetText("Vérifier");
	...
	newFrame.parentFrameText = UnBotBagsHeadFrameSetFontText(fromParent.raceName, fromParent.target, " Voir les accessoires");
	...
	newFrame.title:SetFont([[Fonts\ZYHei.ttf]],20);
	newFrame.title:SetTextColor(1.0,0.8,0,1);
	newFrame.title:SetText("équipement");
	newFrame.title:SetPoint("TOPLEFT",newFrame,"TOPRIGHT",2,-5);
	newFrame.title:SetShadowColor(0,0,0);
	newFrame.title:SetShadowOffset(1,-1);
	newFrame:Show();
	...
	newFrame.parentFrameText = UnBotBagsHeadFrameSetFontText(fromParent.raceName, fromParent.target, " Accessoires d'équipement");
	...
	newFrame.afterRemove = true;
	newFrame.parentFrameText = UnBotBagsHeadFrameSetFontText(fromParent.raceName, fromParent.target, " jeter les accessoires");
	...
	newFrame.afterRemove = true;
	newFrame.parentFrameText = UnBotBagsHeadFrameSetFontText(fromParent.raceName, fromParent.target, " Vendre des accessoires");
	...
	newFrame.title:SetFont([[Fonts\ZYHei.ttf]],20);
	newFrame.title:SetTextColor(1.0,0.8,0,1);
	newFrame.title:SetText("utiliser");
	newFrame.title:SetPoint("TOPLEFT",newFrame,"TOPRIGHT",2,-5);
	newFrame.title:SetShadowColor(0,0,0);
	newFrame.title:SetShadowOffset(1,-1);
	newFrame:Show();
	...
	newFrame.afterRemove = true;
	newFrame.parentFrameText = UnBotBagsHeadFrameSetFontText(fromParent.raceName, fromParent.target, " Utiliser des accessoires");
...
function UpdateUnBotBagsFramePage(bagsFrame)
	...
	bagsFrame.page:SetText("Non. "..tostring(bagsFrame.currentPage).." - "..tostring(overIndex).." Page");
end
	...
	if (bagsFrame.command ~= nil and bagsFrame.command ~= "") then
		if (bagsFrame.command == UnBotExecuteCommand[67]) then
			...
			if (targetName == nil or targetName == "") then
				DisplayInfomation("Vous n'avez actuellement aucun marchand sélectionnéNPCCible.");
				return false;
			end
		end
...
function IsFilterInfo(info)
	local f1,f2 = string.find(info,"équipement");
	if (f1 ~= nil or f2 ~= nil) then
		return true;
	end

	f1,f2 = string.find(info,"détruire");
	if (f1 ~= nil or f2 ~= nil) then
		return true;
	end

	f1,f2 = string.find(info,"vendre");
	if (f1 ~= nil or f2 ~= nil) then
		return true;
	end

	f1,f2 = string.find(info,"utiliser");
	if (f1 ~= nil or f2 ~= nil) then
		return true;
	end
