...
local function GetClassFlag(className)
	if (className == "guerrier") then
		return 1;
	elseif (className == "saint chevalier") then
		return 2;
	elseif (className == "voleur") then
		return 3;
	elseif (className == "Druide") then
		return 4;
	elseif (className == "chasseur") then
		return 5;
	elseif (className == "chaman") then
		return 6;
	elseif (className == "mage") then
		return 7;
	elseif (className == "démoniste") then
		return 8;
	elseif (className == "prêtre") then
		return 9;
	else
		return nil;
	end
...
local function CreateStrategyFrame(name)
	...
	strategyFrame.title:SetText("|cff00ffff"..name.."|r".."Éditeur de politiques");
	...
	strategyFrame.leftText:SetText("stratégie de combat");
	...
	strategyFrame.rightText:SetText("stratégie de non-combat");
...
function StrategyOption_OnEnter(btn, name, use, strategyType, data)
	...
	if (use) then
		GameTooltip:AddLine(" Activé",0,0,1,1);
	else
		GameTooltip:AddLine(" Non activé",1,0,0,1);
	end
	GameTooltip:AddLine(data[3],0,1,0,1);
	GameTooltip:AddLine("Certaines stratégies ne fonctionnent pas pour certaines professions，Même si ces politiques sont activées，Sera également rejeté par les robots，Et certaines stratégies s’excluent mutuellement.",1,0.5,0,1);
	GameTooltip:AddLine(" ",1,1,1,1);
	GameTooltip:AddDoubleLine(" exécuter la commande：",data[1],0,0.85,0.85,0,0.85,0.85);
	if (use) then
		GameTooltip:AddLine(" Clic de souris : Annuler la police",0.65,0.55,0,1);
	else
		GameTooltip:AddLine(" Clic de souris : activer la stratégie",0.65,0.55,0,1);
	end
	GameTooltip:Show();
end
