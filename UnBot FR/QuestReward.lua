...
function ShowQuestReward(name, items)
	...
	if (qrLabel ~= nil) then
		qrLabel:SetText(name.." |cffcccc00Sélectionnez des récompenses pour avoir accompli des tâches|r");
	else
		DisplayInfomation("Panneau de récompense de mission completLabel "..qrFrame:GetName().."TitleLabel Introuvable, échec de définition du nom du joueur "..name);
	end
...
function UnBotShowQuestRewardTips(self, item)
	...
	if (item[2] == true) then
		...
		if (itemName ~= nil and texture ~= nil) then
			...
			GameTooltip:AddLine("Cet accessoire n'est pas apparu dans votre client，Vous devez attendre un moment pour interroger le serveur, puis déplacer à nouveau la souris sur cet accessoire.",1,0,0,1);
		end
	else
		GameTooltip:SetHyperlink("item:"..tostring(item[1])..":0:0:0:0:0:0:0");
	end
	...
	if (self.item[2] == false) then
		GameTooltip:AddLine(" ",1,1,1,1);
		GameTooltip:AddLine("Clic gauche de la souris : Laissez "..item[3].." Sélectionnez cette récompense de mission",1,0,0,1);
	end
	GameTooltip:Show();
end
